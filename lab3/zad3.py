...
def initializeColor(size:int):
    color=numpy.zeros((size,size,3))
    for i in range(size):
        for j in range(size):
            color[i][j]=[random(), random(), random()]
    return color

def analize(tablica, colors):
    ...
    for i in range(len(tablica)-1):
        for j in range(len(tablica)-1):
            ...
            glVertex(tablica[i][j])
            
            glVertex(tablica[i+1][j])
            
            glVertex(tablica[i][j+1])
            glEnd()
